...
export default function StudentLife() {
  ...
  const [activitiesRef, activitiesVis] = useReveal()
  const [photosRef,     photosVis]     = useReveal()
  const [scheduleRef,   scheduleVis]   = useReveal()
    ...
    <div style={{ backgroundColor: '#fff' }}>
      ...
      <section style={{ backgroundColor: '#0A1628', padding: '64px 0 60px' }}>
        <div ref={heroRef} className="wrap">
          <p style={{ fontSize: 10, color: '#B8882A', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 18, ...fx(heroVis, 0) }}>On campus</p>
          <h1 style={{ fontFamily: D, fontWeight: 600, fontSize: 'clamp(40px, 5.5vw, 68px)', color: '#fff', letterSpacing: '-0.01em', lineHeight: 1.02, marginBottom: 18, maxWidth: 560, ...fx(heroVis, 120, 28) }}>
            Life at Fergana<br />Presidential School.
          </h1>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.52)', lineHeight: 1.8, maxWidth: 420, ...fx(heroVis, 240) }}>
      ...
      <section style={{ backgroundColor: '#F5F1E8', borderBottom: '1px solid #E5DFCF', padding: '88px 0' }}>
        <div ref={activitiesRef} className="wrap">
          <h2 style={{ fontFamily: D, fontWeight: 600, fontSize: 'clamp(28px, 3.5vw, 44px)', color: '#0A1628', letterSpacing: '-0.01em', marginBottom: 56, ...fx(activitiesVis, 0) }}>
            Everything on campus
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 80px' }}>
            {lifeItems.map(({ icon: Icon, title, desc }, i) => (
              <div key={title} style={{ display: 'flex', gap: 18, padding: '28px 0', borderTop: '1px solid #DDD8CC', ...fx(activitiesVis, 80 + i * 60) }}>
      ...
      <section style={{ padding: '88px 0' }}>
        <div ref={photosRef} className="wrap">
          <h2 style={{ fontFamily: D, fontWeight: 600, fontSize: 'clamp(28px, 3.5vw, 44px)', color: '#0A1628', letterSpacing: '-0.01em', marginBottom: 36, ...fx(photosVis, 0) }}>
            Campus Moments
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3 }}>
            {photos.map((url, i) => (
              <div key={i} style={{ overflow: 'hidden', ...fx(photosVis, 60 + i * 50) }}>
      ...
      <section style={{ backgroundColor: '#F5F1E8', borderTop: '1px solid #E5DFCF', padding: '88px 0' }}>
        <div ref={scheduleRef} style={{ maxWidth: 600, margin: '0 auto', padding: '0 32px' }}>
          <h2 style={{ fontFamily: D, fontWeight: 600, fontSize: 'clamp(28px, 3.5vw, 44px)', color: '#0A1628', letterSpacing: '-0.01em', marginBottom: 52, ...fx(scheduleVis, 0) }}>
            A Day in the Life
          </h2>
          {schedule.map(({ time, activity }, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 24, padding: '13px 0', borderBottom: i < schedule.length - 1 ? '1px solid #DDD8CC' : 'none', ...fx(scheduleVis, 60 + i * 35) }}>
